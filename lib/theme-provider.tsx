...
export function ThemeProvider({
  ...
  const [theme, setTheme] = useState<Theme>(() => {         // Theme ปัจจุบัน เริ่มต้นจาก localStorage หรือ default
    if (typeof window !== 'undefined') {
      return (localStorage?.getItem(storageKey) as Theme) || defaultTheme
    }
    return defaultTheme
  })
  const mountedRef = useRef(false)                             // สถานะการ mount ของ component

  /**
   * Effect แรก: ตั้งสถานะ mounted เมื่อ component mount
   */
  useEffect(() => {
    mountedRef.current = true                                     // ตั้งสถานะ mounted เป็น true
  }, [])

  /**
   * Effect หลัก: ปรับ CSS classes และ attributes ตาม theme
   * ทำงานเมื่อ theme เปลี่ยน
   */
  useEffect(() => {
    if (!mountedRef.current) return                                 // รอให้ component mount เสร็จก่อน
